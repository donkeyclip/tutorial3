...
const DURATION_CHANGE = "duration-change"; // Mouce/Touch
...
const eltag = document.getElementsByTagName.bind(document);
const elcreate = document.createElement.bind(document);
...
function addMouseUpAndMoveListeners(callbackForUp, callbackForMove) {
  ...
  addListener(mousemove, callbackForMove, {
    passive: false
  });
  addListener(touchmove, callbackForMove, {
    passive: false
  });
}
function removeMouseUpAndMoveListeners(callbackForUp, callbackForMove) {
  ...
  removeListener(mousemove, callbackForMove, {
    passive: false
  });
  removeListener(touchmove, callbackForMove, {
    passive: false
  });
}
...
function initializeOptions(options, _this) {
  ...
  (_options$theme = options.theme) !== null && _options$theme !== void 0 ? _options$theme : options.theme = "donkeyclip";
  ...
  options.buttons.donkeyclip = (() => {
    ...
    if (options.buttons.donkeyclip) return true; // case the request happens from embed url
    // single page clip or iframe

    if (["api.donkeyclip.com", "staging-api.donkeyclip.com"].includes(window.location.host)) return true; // case the request happens from a sourceless iframe
    // outside *.donkeyclip.com

    try {
      var _window$parent, _window$parent$locati;

      if (!((_window$parent = window.parent) !== null && _window$parent !== void 0 && (_window$parent$locati = _window$parent.location) !== null && _window$parent$locati !== void 0 && _window$parent$locati.host.includes("donkeyclip.com"))) return true;
    } catch (e) {
      return false;
    } // else
...
const createSpeedValues = _this => {
  for (const i in _this.options.speedValues) {
    ...
    const selectedClass = "--mcp-selected"; //create the parent li element
...
var donkeyclipListener = (_this => {
  _this.elements.donkeyclipButton.addEventListener("click", () => {
    ...
    window.open("https://".concat(staging, "donkeyclip.com/explore/donkeyclips/").concat(id));
  });
});
...
function add$1(_this) {
  _this.elements.settingsSpeedButtonShow.onclick = _this.elements.settingsSpeedButtonHide.onclick = () => {
    ...
    const includesClass = _this.elements.settingsPanel.className.includes("".concat(_this.name, "-settings-speed-panel"));
...
class Player {
  constructor(options) {
    ...
    if (this.options.type === "scroller") {
      ...
      this.timeBucket = 0;
      this.timeProgress = 0;
      (_this$options$section = this.options.sections) === null || _this$options$section === void 0 ? void 0 : _this$options$section.sort(sortFunc);
    }
  ...
  changeSettings(newOptions, initial) {
    newOptions = initializeOptions({ ...this.options,
      ...newOptions
    }, this);
  ...
  getSectionTime(direction) {
    ...
    if (direction > 0) {
      ...
      (_sectionIndex = sectionIndex) !== null && _sectionIndex !== void 0 ? _sectionIndex : sectionIndex = sortedSections.length - 1;
    } else {
      var _sectionIndex2;
  ...
  changeInitParams(initParams) {
    ...
    definition.props.initParams = initParams; // unmount the previous clip
    ...
    try {
      var _newClip, _newClip$errors;

      newClip = motorcortex.utils.clipFromDefinition(definition);
      if (newClip.nonBlockingErrorClip || (_newClip = newClip) !== null && _newClip !== void 0 && (_newClip$errors = _newClip.errors) !== null && _newClip$errors !== void 0 && _newClip$errors.length) throw "Error: Params Error: Clip cannot be created!";
    } catch (e) {
  ...
  timeFormat(ms) {
    ...
    const diff = ms - timeCache[0]; // If the diff from previous calculated value is less than a second, return the cached result
    ...
    const seconds = minutes % 1 * 60; // By default, JavaScript converts any floating-point number
    ...
    const date = "".concat(h === "00" ? "" : h + ":").concat(m, ":").concat(s);
